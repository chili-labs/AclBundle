...
class ObjectAceManagerFieldTest extends AbstractSecurityTest
{
    ...
    {
        $this->objectmanager->grant($this->object, MaskBuilder::MASK_EDIT, $this->token, 'foo');

        $this->assertTrue($this->manager->isGranted('VIEW', $this->object, 'foo'));
        $this->assertTrue($this->manager->isGranted('EDIT', $this->object, 'foo'));

        $this->objectmanager->overwrite($this->object, MaskBuilder::MASK_VIEW, $this->token, 'foo');
